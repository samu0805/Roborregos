...
void aheadC(){
  ...
  for(int i=1; i>0; i++) {
    ...
    // SPEED_MT = map(c, 0, 35, 190+corregir,90);
    // SPEED_MT2 = map(c2, 0, 35, 190-corregir,90);
    SPEED_MT =150+corregir;
    SPEED_MT2 = 150-corregir;
    SPEED_MT = constrain(SPEED_MT, 120, 255);
    SPEED_MT2 = constrain(SPEED_MT2, 120, 255);
    set_speed(); 
    // PID();
...
void zonaB(){
  for(int i=1;i>0;i++){
    setahead();
    int corregir=corregir_avanza();
    SPEED_MT =90+corregir;
    SPEED_MT2 = 90-corregir;
    SPEED_MT = constrain(SPEED_MT, 50, 255);
    SPEED_MT2 = constrain(SPEED_MT2, 50, 255);
    set_speed(); 
    if(c2>240 || c2>240){
      break;
    }
