...
void ahead(){
  while (true) {
    distancia();
    if(c >= 90 || c2>=90 || distance<15){
      break;
    }
    setahead();
    // Mapeo de velocidad basado en el contador de pulsos
    int corregir=corregir_avanza();
    SPEED_MT = map(c, 0, 35, 190+corregir,140);
    SPEED_MT2 = map(c2, 0, 35, 190-corregir,140);
    set_speed(); 
    // PID();
...
void back(){
  ...
  while (true) {
    ...
    set_speed(); 
    // PID();
    if (c >= 110 ) {// Para detener el bucle cuando se alcanzan los 38 pulsos
      digitalWrite(IN1_MT,0);
      digitalWrite(IN2_MT,0);
      digitalWrite(IN3_MT,0);
      digitalWrite(IN4_MT,0);
